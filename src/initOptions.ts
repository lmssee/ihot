...
async function initOptions() {
    ...
    /** 获取配置文件 */
    if (!op || Object.keys(op).length == 0) return console.log(Color.darkBlue('您未配置配置文件，即将为您自动启动默认配置'));
    // 自定义配置处理
    op.watch = optionToArray(op.watch);
    op.skip = optionToArray(op.skip);
    op.skip = !op.skip ? null : new RegExp(op.skip.concat(hotData.configFileName).join("\|").replace(/\./mg, "\\\."));
    /** 
     * 
     * 临时变量，处理执行的代码
      */
    const _temp = op.code ? op.code.trim().replace(/\s{2,}/mg, ' ').split(' ') : [];
