...
export async function initConfig(
  data: string | true | { [key: string]: string | true | string[] }
) {
  ((data == true || typeof data == "string") && (await noExtension(), 1)) ||
    //@ts-ignore
    (data.json && (await testFileExist(), 1)) ||
    //@ts-ignore
    (data.ts && (await testFileExist("ts"), 1)) ||
    (await testFileExist("js"));
}
/** 没有指定配置文件类型  */
async function noExtension() {
  const isExist = ["json", "js", "ts"].find((currentEle: string) => {
    const fileName = `lmconfig.${currentEle}`,
...
async function testFileExist(extension: string = "json") {
  const fileName = `lmconfig.${extension}`,
    fileInfo = fileExist(fileName);
...
function createJsonConfigFile() {
  ...
  writeFileSync("lmconfig.json", JSON.stringify(data, null, 2), {
    encoding: "utf-8",
    flag: "w",
  });
}
...
()=> ({
  ...
  writeFileSync("lmconfig.ts", data, { encoding: "utf-8", flag: "w" });
}
...
()=> ({
  ...
  writeFileSync("lmconfig.js", data, { encoding: "utf-8", flag: "w" });
}
