...
function initArgData(v: any) {
    ...
    typeof v.base == 'string' && (initArg.base = v.base),
        ...
        // 配置跳过文件
        (typeOf(v.skip) == 'string' && (v.skip = [v.skip]), 0) || (typeOf(v.skip) == 'array' && (initArg.skip = new RegExp(v.skip.join("\|").replace(/\./mg, "\\\.")))),
        // 配置其余参数
        (typeof v.args == 'string' && (v.args = [v.args]), 0) || (typeOf(v.args) == 'array' && (initArg.args = v.args));
}
